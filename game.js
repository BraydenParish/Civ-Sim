...
const SPEED_PRESETS = [0,1,2,5];
const isBrowser = typeof window !== 'undefined' && typeof document !== 'undefined';

let SPEED = 1.0;
let TIME = 0;
let lastRequestedSpeed = SPEED;
globalThis.SPEED = SPEED;

function calculateStepSegments(speed) {
    if (speed <= 0) return { iterations: 0, subStep: 0 };
    const iterations = Math.max(1, Math.ceil(speed));
    return { iterations, subStep: speed / iterations };
}

function applySpeed(newSpeed, buttons = []) {
    SPEED = newSpeed;
    globalThis.SPEED = SPEED;
...
function setSpeed(s, buttons) {
    if (!buttons && typeof document !== 'undefined') buttons = Array.from(document.querySelectorAll('#controls button'));
...
class Game {
    ...
    loop() {
        ...
        const realS = SPEED;
        for(let i=0;i<iterations;i++) { SPEED=subStep; this.update(); }
        SPEED = realS;
        requestAnimationFrame(()=>this.loop());
    }
}
...
function initControls() {
    ...
    buttons.forEach((btn, idx) => btn.value = SPEED_PRESETS[idx]);
    const pauseBtn = document.getElementById('btn-pause');

    const updatePauseLabel = () => { if(pauseBtn) pauseBtn.textContent = SPEED === 0 ? 'RESUME' : 'PAUSE'; };
    const setAndRemember = (val) => { setSpeed(val, buttons); updatePauseLabel(); };

    if(pauseBtn) {
        pauseBtn.addEventListener('click', () => {
            if(SPEED === 0) {
                setAndRemember(lastRequestedSpeed || 1);
            } else {
    ...
    buttons.slice(1).forEach((btn, idx) => {
        const target = SPEED_PRESETS[idx+1];
        btn.addEventListener('click', () => { lastRequestedSpeed = target; setAndRemember(target); });
    });

    applySpeed(SPEED, buttons);
    updatePauseLabel();
    return buttons;
}

if(isBrowser) {
    const boundButtons = initControls();
    applySpeed(SPEED, boundButtons);
    new Game();
}
